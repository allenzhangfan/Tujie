...
public interface HistoryVideoContract {
    ...
    interface View extends IView {
        void refreshData();
        void getVideoSucess(HistoryVideoResponseEntity entity);
        void getVideoFAiled();
    }

    //Model层定义接口,外部只需关心Model返回的数据,无需关心内部细节,即是否使用缓存
    interface Model extends IModel {
        Observable<HistoryVideoResponseEntity> getlistHistoryVideo(HistoryVideoRequestEntity okcRequestEntity);
    }
}
